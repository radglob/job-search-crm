...
class InstallRequirement(object):
    ...
    def is_pinned(self):
        ...
        return (
            len(specifiers) == 1 and next(iter(specifiers)).operator in ("==", "===")
        )
            ...
            if self.editable:
                ...
                for root, dirs, files in os.walk(base):
                    ...
                    for dir in list(dirs):
                        ...
                        # environment
                        if (
                            os.path.lexists(os.path.join(root, dir, "bin", "python"))
                            or os.path.exists(
    ...
    def update_editable(self, obtain=True):
        ...
        if backend:
            ...
            assert 0, (
                "Unexpected version control type (in %s): %s" % (self.link, vc_type)
            )
    ...
    def rollback_uninstall(self):
        if self.uninstalled:
            ...
            logger.error("Can't rollback %s, nothing uninstalled.", self.name)

    def commit_uninstall(self):
        if self.uninstalled:
            self.uninstalled.commit()
        elif not self.nothing_to_uninstall:
            logger.error("Can't commit %s, nothing uninstalled.", self.name)
        ...
        try:
            ...
            if self.use_user_site:
                if dist_in_usersite(existing_dist):
                    ...
                    running_under_virtualenv() and dist_in_site_packages(existing_dist)
                ):
                    raise InstallationError(
