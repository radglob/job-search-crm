...
def get_build_platform():
    ...
    if sys.platform == "darwin" and not plat.startswith("macosx-"):
        try:
            ...
            return "macosx-%d.%d-%s" % (
                int(version[0]), int(version[1]), _macosx_arch(machine)
            )
...
def dist_factory(path_item, entry, only):
    ...
    return (
        distributions_from_metadata if is_meta else find_distributions if not only
        and _is_egg_path(entry) else resolve_egg_link if not only
        and lower.endswith(".egg-link") else NoDists()
    )
