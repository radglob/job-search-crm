...
def sign(payload, keypair):
    ...
    return {
        "recipients": [
            {"header": native(encoded_header), "signature": native(encoded_signature)}
        ],
...
def verify(jwsjs):
    ...
    for recipient in recipients:
        ...
        assertTrue(
            header["alg"] == ALG, "Unexpected algorithm {0}".format(header["alg"])
        )
        if "alg" in header["jwk"] and "kty" not in header["jwk"]:
            header["jwk"]["kty"] = header["jwk"]["alg"]  # b/w for JWK < -08
        assertTrue(
            header["jwk"]["kty"] == ALG,  # true for Ed25519
